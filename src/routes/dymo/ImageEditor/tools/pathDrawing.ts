...
const pathDrawing = (fabricCanvas: Fabric.Canvas, fabric: any) => {
    ...
    let pointer;
    let updatedPath: (string|number)[] = [];
    let isMouseDown = false;
    let isDrawingCurve = false;
    let rememberX: number;
    let rememberY: number;


    fabricCanvas.on('mouse:down', (o) => {
        if (!fabricCanvas.get("isDrawingPathMode")) return
    ...
    fabricCanvas.on('mouse:up', (o) => {
        if (!fabricCanvas.get("isDrawingPathMode")) {
            isMouseDown = false
    ...
    const cancelDrawing = () => {
        ...
        if (pathToDraw && pathToDraw.path.length > 1) {
