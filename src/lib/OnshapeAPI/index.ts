...
export default class OnshapeAPI {
	...
	public async ComparePartstudios(
		documentId: string,
		wv: WVM.W | WVM.V,
		wvId: string,
		elementId: string,
		otherRevision: OtherRevArgs): Promise<BTRootDiffInfo> {

		const opts: GetOpts = {
			d: documentId,
			e: elementId,
			query: otherRevision as {[key: string]: string},
			resource: 'partstudios',
			subresource: 'compare',
		};
		opts[wv] = wvId;
		return (await this.get(opts as any)) as BTRootDiffInfo;
	}
